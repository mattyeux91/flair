...
>
> **Limite connue, assumée.** `MatchSystem` note la moyenne des compétences d'un effectif quand le budget salarial en contraint le total : concentrer est légèrement gagnant (14 joueurs à 60,3 de qualité moyenne pour le meilleur club, 18 à 50,8 pour le dernier). La correction appartient au moteur de match — ne noter que les onze meilleurs — et demande `PositionAffinity`, hors périmètre Phase 2.
>
> **Reste à faire dans la phase :** perception/scouting (lot suivant, cf. note ci-dessous), puis marché des transferts multi-tours et agents PNJ (`14-` §5), puis `marketInflationTarget` et son régulateur — la moitié « inflation dans la cible » du critère de sortie.
