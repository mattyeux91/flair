...
- **`System`** (interface) — `id()`, `reads()`/`writes()`/`removes()` (composants lus, mutés en place via `set()`, ou retirés via `remove()` — deux systèmes distincts sur `writes()`/`removes()` sont détectés mécaniquement, voir la section Dépendances), `subscribesTo()` (types d'événements écoutés), `handle(DomainEvent, SystemContext)`, `update(SystemContext)`.
...
- **`Balance`** — les leviers d'équilibrage ; `developmentRate` (multiplicateur global de la progression naturelle, lu par `Football\PlayerDevelopmentSystem` — premier champ réellement consommé) est le premier. `trainingRate`/`injuryBaseHazard`/`marketInflationTarget` (`12-` §6) rejoindront cette classe quand un système les lira réellement.
...
- **`TrainingEffect`** — point d'extension pour un futur `TrainingSystem` (hors périmètre de la phase en cours) : un multiplicateur `[0.5, 2.0]` déjà borné par son producteur, lu par `PlayerDevelopmentSystem` avec un défaut neutre (1.0) tant qu'aucun système ne l'écrit — plomberie sans consommateur, comme `TickContext.intents`.
- **`PlayerRetired`** — le seul Fait émis par `RetirementSystem` : irréversible (`docs/16-` §2), la dérive quotidienne des attributs ne franchit aucun seuil décisionnel.
