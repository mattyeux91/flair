...
Pipeline — **une seule écriture, `Football\FootballPipeline`** (`bin/demo.php`, le harness et le futur `host` la lisent tous ; `Football\PipelineInvariantsTest` en porte l'assertion dorée, seconde écriture délibérée). L'**ordre d'exécution en est dérivé** par `SystemGraph` : `declaration()` est la liste écrite à la main, `systems()` le résultat du tri. Les deux coïncident aujourd'hui, et un test le prouve — sinon la liste ne dirait plus la vérité sur ce qui s'exécute. Ordre obtenu : `FacilitiesSystem`, `YouthIntakeSystem`, `SquadSystem`, `TrainingSystem`, `RetirementSystem`, `FinanceSystem`, `PlayerDevelopmentSystem`, `CalendarSystem`, `MatchSystem`, `CompetitionSystem`, `ContractSystem` — `FacilitiesSystem` en tête (ses deux lecteurs ouvrent le pipeline), les facteurs avant les effets (`TrainingSystem` avant `PlayerDevelopmentSystem`), `YouthIntakeSystem` en tête (canal 1 pour les joueurs promus), `FinanceSystem` après `RetirementSystem` (il lit `Contract`, que la retraite retire), puis le trio calendrier/match/classement en fin de pipeline (`MatchSystem` avant `CompetitionSystem`, obligatoire pour le canal 1 ci-dessus), et `SquadSystem`/`ContractSystem` qui encadrent le tout (décider en queue, appliquer en tête au tick suivant).
...
- **Limite connue — requalifiée en dette à solder avant le marché (2026-08-04).** `MatchSystem` note la **moyenne** des compétences d'un effectif quand le budget en contraint le **total** : concentrer le budget sur peu de bons joueurs est donc légèrement avantageux (14 joueurs à 60,3 de qualité moyenne pour le meilleur club, 18 à 50,8 pour le dernier). Un plancher d'effectif a été essayé pour le corriger et **mesuré comme nuisible** — Gini inchangé, rotation du top 5 retombée à 49,2 % — donc retiré. Cette entrée affirmait ensuite que la vraie correction « demande `PositionAffinity`, hors périmètre » : c'est faux dans les deux sens. Ne noter que les onze meilleurs ne demande aucune notion de poste (un tri sur l'agrégat que `ratings()` calcule déjà par joueur), et ce n'est pas un raffinement esthétique du moteur de match mais une **incitation économique inversée** — sous la moyenne, la valeur marginale d'un joueur sous le niveau de l'effectif est négative. À corriger avant le marché des transferts, sous peine de calibrer des prix contre un signal qui s'inversera. Voir `docs/15-roadmap.md` §4 Phase 2. (`Football\Support\WageModel` mentionne `PositionAffinity` pour un besoin différent et toujours valide : une qualité fine par poste.)
