...
src/Core/
  ...
  Support/      PRNG déterministe et arithmétique 32 bits
  Ruleset.php   règles paramétriques versionnées (value object minimal)
...
Value object minimal : `public string $version`. Volontairement pauvre pour l'instant — `docs/12-` §6 décrit un bundle riche (compétitions, fenêtres de transfert, balance...), mais rien ne lit encore ces champs, donc rien n'est écrit. Il rejoindra le contenu réel au fur et à mesure qu'un système du domaine football en aura besoin. À la racine de `Core/` plutôt que dans une famille dédiée : ni `Pipeline` ni `Simulation` ne doivent dépendre l'un de l'autre pour ce type, et les deux en ont besoin.
...
## `Ruleset` et `TickContext.intents` — de la plomberie sans consommateur

Les deux existent et transitent jusqu'à `SystemContext::ruleset()`/`::intents()`, mais **aucun système n'existe encore pour les lire**. C'est voulu : le kernel expose la donnée avant qu'un système en ait besoin, comme il l'a fait pour `rng()` avant le premier système concret. Le premier système du domaine football (`docs/15-` §4) sera aussi le premier consommateur réel de l'un des deux.
