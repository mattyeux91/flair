...
**Stade actuel : Phase 0 en cours (`15-roadmap.md`).** `packages/kernel/` existe (`composer.json`, PHPUnit, PHPStan niveau max — tous verts). `src/Core/` (générique, zéro dépendance football) est maintenant rangé par famille :
  ...
  - `Core/Ruleset/` — famille (comme `Pipeline`/`Simulation`), imbriquée par sous-domaine plutôt qu'une liste plate de scalaires (`12-` §6 : `competitions`, `transferWindows`, `contracts`, `finance`, `balance`). `Ruleset` (racine : `version` + une propriété par groupe), `Balance` (`developmentRate`, lu par `Football\PlayerDevelopmentSystem` via `ruleset()->balance->developmentRate` ; `trainingRate`, même rôle mais pour `Football\TrainingSystem` ; `retirement: RetirementBalance` et `playerDevelopment: PlayerDevelopmentBalance`, une classe dédiée par système plutôt qu'une classe partagée — un système ne dépend jamais des leviers d'un autre, même principe que `reads()`/`writes()` sur `System`), `RetirementBalance` (âge d'éligibilité à la retraite, poids âge/fragilité dans la probabilité de retraite, lus uniquement par `RetirementSystem`) et `PlayerDevelopmentBalance` (forme de g(age) et un multiplicateur de déclin post-pic **par catégorie de compétence**, lus uniquement par `PlayerDevelopmentSystem` — jamais en dur dans le code des systèmes, chaque champ documenté individuellement) ; ni `Balance`/`RetirementBalance`/`PlayerDevelopmentBalance` dans `Pipeline` ni dans `Simulation` pour ne pas créer de dépendance entre les deux, qui en ont besoin tous les deux.
...
Le vieillissement est scindé en **deux systèmes à responsabilité unique** plutôt qu'un seul, pour que la progression future (entraînement) n'ait jamais deux writers sur les mêmes composants (`13-` §2 — l'invariant qu'un test mécanique vérifie désormais, `Football\PipelineInvariantsTest`) :
  - `RetirementSystem` — purement périodique, **seul système qui `remove()`** `PlayerPotentials` et les trois composants de compétences : retraite probabiliste au-delà d'un âge d'éligibilité (poids âge/fragilité, `RetirementBalance`), émet `PlayerRetired`.
  - `PlayerDevelopmentSystem` — purement périodique, **seul writer** (`set()`) des trois composants de compétences : progression/déclin de chaque attribut via un taux annuel converti en probabilité quotidienne d'un pas de ±1 — évite qu'un taux journalier fractionnaire ne s'arrondisse toujours à zéro ; un `ageFactor` calculé séparément par catégorie à partir de son propre pic, et une pente de déclin post-pic différente par catégorie via `PlayerDevelopmentBalance` — le physique décline plus vite que le technique, lui-même plus vite que le mental. Lit `TrainingEffect` (composant, `Football/Components/`) — un multiplicateur `[0.5, 2.0]` déjà borné par son producteur, défaut neutre (1.0) quand absent (joueur sans club) ; post-pic, le modificateur s'applique par sa réciproque (`1/x`, bijection de `[0.5, 2.0]` sur lui-même) pour qu'un bon encadrement ralentisse le déclin au lieu de l'accélérer.

Premier consommateur **et** producteur de `TrainingEffect` : `TrainingSystem` (purement périodique, aucun RNG — fonction déterministe, pas de tirage). Ne modélise que `h(entraînement)` de la formule complète `modif = clamp(h × i(temps de jeu) × j(moral), 0.5, 2.0)` (`14-` §2) — `i`/`j` dépendent d'infrastructures qui n'existent pas encore (`MatchSystem`, un composant `Morale`) et seront, le jour venu, des composants-facteurs **séparés** composés indépendamment par `PlayerDevelopmentSystem`, jamais fusionnés ici (un seul writer par composant, `13-` §2). Domaine club minimal pour ce porter ce seul facteur, volontairement réduit face au catalogue complet de `12-` §3 (`Club { name }`, `Facilities { quality }` exprimée directement sur l'échelle `[0.5, 2.0]`, `SquadMembership { clubId }` sur l'entité joueur — pas de `Finances`/`Squad`/`Reputation`/`FanBase`/`BoardExpectations`, pas de worldgen, clubs créés à la main). `Balance::$trainingRate` (défaut 1.0) est le multiplicateur global de calibration, même rôle que `developmentRate`. **Seul writer** de `TrainingEffect`, ne lit jamais `PlayerPotentials`/les compétences — aveugle à ce qui consomme son résultat, symétrique de `PlayerDevelopmentSystem` qui est aveugle à sa provenance. Doit s'exécuter avant `PlayerDevelopmentSystem` dans le pipeline (`RetirementSystem`/lui n'ont pas d'ordre imposé entre eux) — pipeline déclaré : `TrainingSystem`, `RetirementSystem`, `PlayerDevelopmentSystem`.
