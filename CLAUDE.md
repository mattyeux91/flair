...
`src/Football/` n'existe pas encore — apparaîtra avec le premier composant du domaine (`Contract`, `PlayerSkills`...). Le socle générique du noyau (ECS, messagerie, pipeline, `step()`) est posé au complet et exécutable de bout en bout. Reste à poser avant la suite : le premier système concret (domaine football, `15-` §4) — jusque-là, `Ruleset`/`TickContext.intents` sont de la plomberie sans consommateur.
