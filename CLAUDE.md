...
- Pipeline déclaré : `FacilitiesSystem`, `YouthIntakeSystem`, `SquadSystem`, `TrainingSystem`, `RetirementSystem`, `FinanceSystem`, `PlayerDevelopmentSystem`, `CalendarSystem`, `MatchSystem`, `CompetitionSystem`, `ContractSystem`. `SquadSystem` (application) et `ContractSystem` (décision) encadrent tout le reste : le décideur doit lire les compétences et `Finances`, donc passer après leurs writers ; l'applicateur doit écrire `SquadMembership` avant `TrainingSystem`/`MatchSystem`. Aucun ordre ne satisfait les deux — décider tard, appliquer tôt au tick suivant.
