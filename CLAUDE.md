...
- **ECS strict, aucun sous-type.** `Player`, `Club`, `City` n'existent **pas** comme classes. Une entité est un entier ; « joueur » est le nom informel d'une entité qui porte `PlayerSkills` + `Contract` + `Fitness`. Les composants sont `readonly`, en lecture seule hors de leur système propriétaire. (`12-` §1, §2)
- **Vérité cachée vs perception.** `PlayerSkills`/`Potential` (la vérité) ne sont jamais exposés à un client. Ce qu'un observateur croit est **dérivé** à la lecture (bruit déterministe, fonction de `observerId`/`subjectId`/`observationCount`), jamais stocké. (`12-` §4)
...
**`src/Football/` existe désormais** (namespace `Flair\Kernel\Football`) : premier système concret, le vieillissement (`15-` §4, le plus autonome des cinq systèmes de la Phase 0). `Person` (identité minimale, `birthDate: SimDate`), `PlayerSkills` (les 12 attributs de champ, `12-` §5), `Potential` (`ceiling`/`peakAge`/`growthRate`/`fragility`, `14-` §2), `PlayerRetired` (le seul Fait émis — irréversible, `16-` §2), `AgingSystem` (purement périodique : retraite probabiliste au-delà d'un âge d'éligibilité qui retire `PlayerSkills`/`Potential`, sinon progression/déclin des 12 attributs via un taux annuel converti en probabilité quotidienne d'un pas de ±1 — évite qu'un taux journalier fractionnaire ne s'arrondisse toujours à zéro). Simplifications assumées et documentées dans la classe : `ceiling` partagé entre attributs, pas de potentiel révélé progressivement (dépend du moteur de match), aucun modificateur d'entraînement (le futur système `entraînement` composera sans modifier `AgingSystem`, OCP) — premier jet à calibrer en Phase 1, pas des valeurs équilibrées.
