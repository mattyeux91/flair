...
**Ordre des lots restants, révisé le 2026-08-04 (`15-` §4 Phase 2 pour le raisonnement complet)** — trois lots, dans cet ordre :
...
2. **Perception/scouting**, périmètre réduit : rôle scout semé au genesis, composant d'emploi distinct de `SquadMembership`, `signedOn` sur `Contract`, fonction pure de bruit dans `Football\Support\`, unique bascule `ContractSystem::quality()` — il lit aujourd'hui la vérité cachée, **simplification de périmètre assumée, pas une affirmation de conception** (ce n'est jamais « le club » qui évalue mais une personne, dont la compétence détermine l'erreur). **Aucun mécanisme d'observation** : `observationCount` est indexé par paire (observateur, sujet), donc en O(scouts × joueurs) si on le stocke — on le dérive de l'ancienneté au club, et « qui observe qui » part en Phase 5 avec le jeu d'agent. `observerId` doit être une **personne** (`Person` + rôle, jamais un attribut de `Club`, cf. `docs/12-` §1) — aucun rôle non-joueur n'existe encore ; lire `docs/12-modele-du-monde.md` §4 avant d'y toucher.
...
- Le domaine football (`src/Football/`) : vieillissement (retraite + progression des compétences), entraînement, intake de jeunes (ferme la boucle démographique), calendrier + match L0 (Dixon-Coles) + classement, grand livre monétaire + installations + contrats (Phase 2, ci-dessus).
